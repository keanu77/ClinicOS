...
export default function ClinicSchedulePage() {
  ...
  const [activeTab, setActiveTab] = useState<ClinicType>(
  ...
  const sportsSlots = slots.filter(
    (s) => s.clinicType === ClinicType.SPORTS_MEDICINE,
  );
  ...
  const handleCopyMonth = async (
    ...
    try {
      const result = await apiPost<{ copied: number }>(
        "/clinic-schedule/slots/copy-month",
        {
          sourceYear,
          sourceMonth,
          targetYear,
        ...
        actions={
          canManage ? (
            <>
              ...
              <Button className="btn-lift" onClick={handleAdd}>
